...
// This event is run if a guild is deleted or is deemed unavailable due to an outage from Discord. This listener is
// attached directly on `ws` because the game should end regardless of whether the guild is cached.
@ApplyOptions<Listener.Options>({ event: GatewayDispatchEvents.GuildDelete, emitter: 'ws' })
export class RawGuildDeleteListener extends Listener {
